...
class NoobAIEngine:
        ...
        with perf_monitor.time_section("image_generation"):
            ...
            def callback_on_step_end(pipe, step_index, timestep, callback_kwargs):
                ...
                if self.enable_dora and self.dora_loaded and self.pipe is not None:
                    ...
                    elif current_step == self.dora_start_step - 1 and self.dora_start_step > 1:
                        # Activate DoRA one step early so it's active FOR the target step
                        # (callback runs AFTER step completes, so we activate at step N-1 to be ready for step N)
                        self.pipe.set_adapters(["noobai_dora"], adapter_weights=[self.adapter_strength])
    ...
    def teardown_engine(self):
        ...
        try:
            ...
            try:
                self._device = None
                reset_success.append('_device')
            except Exception as e:
                try:
                    self.__dict__['_device'] = None
                    reset_success.append('_device (via __dict__)')
                except Exception as e2:
                    reset_failures.append(f'_device: {e2}')
